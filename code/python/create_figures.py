...
DATA_PATH = ROOT / "data" / "processed" / "german_segments.csv"
MODEL_DIR = ROOT / "output" / "models" / "gbert_label_classifier"
...
def label_distribution(data):
    ...
    title_block(draw, "German Segments: Label Balance", "Training and test rows by simplified label")
...
def confusion_matrix(metrics):
    ...
    draw = ImageDraw.Draw(image)
    title_block(draw, "Test Confusion Matrix", "Rows are actual labels; columns are model predictions")
...
def prediction_summary(predictions):
    ...
    title_block(draw, "Prediction Summary", f"Correct test predictions: {accuracy:.1%}")
...
def main():
    ...
    print("Created figures in output/figures.")
