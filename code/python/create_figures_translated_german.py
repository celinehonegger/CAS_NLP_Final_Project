...
DATA_PATH = ROOT / "data" / "processed" / "translated_german_segments.csv"
MODEL_DIR = ROOT / "output" / "models" / "gbert_merged_label_classifier" / "translated_german_evaluation"
METRICS_PATH = MODEL_DIR / "metrics.json"
PREDICTIONS_PATH = MODEL_DIR / "predictions.csv"
FIGURE_DIR = ROOT / "output" / "figures" / "translated_german"
...
def label_distribution(data):
    ...
    title_block(draw, "Translated French Segments: Label Balance", "Rows by merged label and original split")
...
def training_history(metrics):
    ...
    title_block(
        draw,
        "Translated French Evaluation",
        f"Accuracy {metrics['accuracy']:.3f}; macro F1 {metrics['macro_f1']:.3f}; loss {metrics['loss']:.3f}",
    )
    ...
    if history:
        ...
        for i, (label, value) in enumerate(cards):
            x = 90 + i * 315
            y = 260
            draw.rounded_rectangle((x, y, x + 260, y + 210), radius=10, fill="#ffffff")
            draw.text((x + 28, y + 36), label, fill=MUTED, font=SMALL_FONT)
            draw.text((x + 28, y + 92), value, fill=INK, font=font(42, bold=True))
        note = "Evaluation uses the merged-label GBERT model on German translations of the French segments."
        for i, line in enumerate(wrap_text(draw, note, LABEL_FONT, 1050)):
            draw.text((90, 560 + i * 28), line, fill=INK, font=LABEL_FONT)

    image.save(FIGURE_DIR / "evaluation_summary.png")
...
def confusion_matrix(metrics):
    ...
    draw = ImageDraw.Draw(image)
    title_block(draw, "Translated French Confusion Matrix", "Rows are actual labels; columns are model predictions")
...
def prediction_summary(predictions):
    ...
    title_block(draw, "Translated French Prediction Summary", f"Correct predictions: {accuracy:.1%}")
...
def main():
    ...
    print("Created translated German evaluation figures in output/figures/translated_german.")
